...
extern "C" int fl_loader_entry(const FlRuntimeRequest* request, FlRuntimeResult* result) {
    ...
    if (log != nullptr) {
        fprintf(log, "loader=libfl_app64 stage=%s target=%s pid=%d entry=%s\n",
            request->stage, request->target_process, request->target_pid, request->entrypoint);
        fprintf(log, "loader_phase=jvm_bind status=ok detail=stub-app-loader\n");
        fprintf(log, "loader_phase=plan_ingest status=ok detail=plan=%s\n",
            request->plan_path != nullptr ? request->plan_path : "none");
        fprintf(log, "loader_phase=classloader_prepare status=ok detail=appHook placeholder handoff\n");
    ...
    snprintf(result->message, sizeof(result->message),
        "libfl_app64 invoked for %s pid=%d entry=%s payload=%zu seed=%zu",
